...
async function loadPortfolioData() {
    try {
        ...
        const localData = localStorage.getItem('adele_portfolio_data_v15');
        let data;
        ...
        if (localData) {
            ...
            const response = await fetch('projects.json?v=15');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            data = await response.json();
            // Save to local storage for future instant loads and admin edits
            localStorage.setItem('adele_portfolio_data_v15', JSON.stringify(data));
        }
