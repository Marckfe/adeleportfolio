...
async function loadPortfolioData() {
    try {
        ...
        const localData = localStorage.getItem('adele_portfolio_data_v21');
        let data;
        ...
        if (localData) {
            ...
            const response = await fetch('projects.json?v=21');
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            data = await response.json();
            // Save to local storage for future instant loads and admin edits
            localStorage.setItem('adele_portfolio_data_v21', JSON.stringify(data));
        }
